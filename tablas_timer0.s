...
PSECT resVect, class=CODE, abs, delta=2
 ...
 setup: ;llama a la subrutinas de configuración
    ...
    movlw 10 ;cantidad de veces a ejecutar el contador de segundos
    movwf contador
 ...
 ;------------------ loop---------------------   
 loop:
    call contador_tmr0_segundos
    
    btfsc PORTE, 0 ;comprueba que el pin 0 del puerto B este presionado (1) si es así ejecuta la siguiente instrucción de lo contrario la salta
    call inc_portC ; llama a la subrutina de incremento del puerto C
    
    btfsc PORTE, 1 ;chequeo pin 1
...
;---------------------subrutinas setup-------------------
 config_io:
    ...
    movwf TRISB ;  configura los 4 bits más significativos como entradas
    movlw 0b11110000
    movwf TRISA
    movlw 0b11110000
    movwf TRISC
    ...
    bsf	TRISE, 0   ; Se configuran 2 pines del puerto E como entradas
    bsf TRISE, 1
    ...
    clrf PORTA	    ;Limpiamos los puertos
    clrf PORTB
    clrf PORTC
    clrf PORTD
    clrf PORTE
...
contador_tmr0_segundos: ;tiempo de ejecucion 0.1s
    btfss  T0IF ;salta cuando es T0IF devuelve 1
    goto $-1
    call reinicio_tmr0 ;llama a la función que reinicia el tmr0
    incf PORTB
    decfsz contador ;decrementa el valor del contador a 1 (contador ahora tiene que realizar n-1 vueltas para llegar a 1s), salta la 
		    ;siguiente linea si devuelve 0
    goto $+3 
    call reinicio_contador
    incf PORTA
...
inc_portC:
    ...
    btfsc PORTE, 0 ; (anti-rebote)comprueba si el boton que llamo a la rutina dejo de ser presionado
...
alarma:
    movf PORTA, W
    subwf PORTC, W
    btfss STATUS, 2 ;chequea si la bandera Z se ha levantado
    goto $+6	
    clrf PORTA
    call reinicio_tmr0
    call reinicio_contador
